...
async function loadData() {
  ...
  badge.textContent = DATA.updated_at
    ? `Patch ${DATA.patch || "?"} · ${new Date(DATA.updated_at).toLocaleDateString("es")}`
    : "Datos de ejemplo (sin actualizar)";
}

// ------------------------------------------------------------------
// Plugin: dibuja "LANE · %" directamente sobre cada porción del pastel
// ------------------------------------------------------------------
const sliceLabelPlugin = {
  ...
  afterDatasetDraw(chart) {
    ...
    meta.data.forEach((arc, i) => {
      ...
      const lane = LANES[i];
      ctx.fillText(lane.id, x, y - 7);
      ctx.font = "600 11px 'JetBrains Mono', monospace";
      ctx.fillText(`${lane.pct}%`, x, y + 8);
      ctx.font = "700 12px 'JetBrains Mono', monospace";
    });
    ctx.restore();
  },
...
// ------------------------------------------------------------------
// Tarjeta de perfil OP.GG (debajo del gráfico)
// ------------------------------------------------------------------
const OPGG_PROFILE = {
  name: "Ronnin",
  tag: "#1999",
...
function renderProfileCard() {
  ...
  if (!el) return;
  const p = (DATA && DATA.profile) || OPGG_PROFILE;
  el.innerHTML = `
    <img class="pc-icon" src="${p.icon}" alt="${p.name}" loading="lazy" />
    <div>
      <div class="pc-name">${p.name}<span style="color:var(--text-dim)">${p.tag}</span></div>
      <div class="pc-region">${p.region} · soloQ</div>
    </div>
    <div class="pc-stats">
      <div class="pc-stat">Elo<strong>${p.rank} · ${p.lp} LP</strong></div>
      <div class="pc-stat">Winrate<strong class="${wrClass(p.winrate)}">${p.winrate}%</strong></div>
      <div class="pc-stat">Partidas<strong>${p.wins}W ${p.losses}L</strong></div>
    </div>
    <a class="pc-link" href="${p.url}" target="_blank" rel="noopener">Ver en OP.GG →</a>`;
}
...
function renderBuildTab(c) {
  return `
    <div class="panel">
      <h2>Cómo jugarlo</h2>
      ${c.playstyle ? `<p class="icon-caption">${c.playstyle}</p>` : "<p class='icon-caption'>Sin datos todavía.</p>"}
    </div>`;
}
