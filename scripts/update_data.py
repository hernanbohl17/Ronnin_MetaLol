...
ITEMS_LOCAL = {
    ...
    6697: "Hybris", 6673: "Arco inmortal",
}
...
STATIC_KNOWLEDGE = {
    ...
    "Graves": {
        "combo": {"keys": "AA → E → AA → AA (el E resetea 1 disparo del arma)", "description": "Pega un AA, usa E para desplazarte y recargar un disparo, y encadena AA-AA de inmediato: así conectás 3 disparos antes de tener que recargar, en vez de los 2 normales."},
        "playstyle": "Jungla de farmeo agresivo: limpia rápido, invade con tu rango de burst y busca peleas 1v1 cortas apoyado en el true damage y el sustain de tus runas.",
        "runes": {"primary": [8128, 8143, 8138, 8135], "secondary": [9104, 9111], "note": "Cosecha oscura es la keystone más jugada (68% de partidas, ~50.2% WR) según Onetricks/Mobalytics, patch 16.13-14."},
        "rune_pages": [
            {
                "label": "Cosecha oscura (más jugada)",
                "primary": [8128, 8143, 8138, 8135],
                "secondary": [9104, 9111],
                "note": "68% de las partidas la usan. Primaria Dominación: Cosecha oscura + Impacto repentino + Recuerdos macabros + Cazatesoros. Secundaria Precisión: Leyenda: Presteza + Triunfo.",
            },
            {
                "label": "Juego de pies (alternativa)",
                "primary": [8021, 9111, 9104, 8014],
                "secondary": [8304, 8330],
                "note": "22% de las partidas. Primaria Precisión: Juego de pies + Triunfo + Leyenda: Presteza + Golpe de gracia. Secundaria Inspiración: Calzado mágico (ahorra 300 de oro) + Tónico triple.",
            },
        ],
        "build": {"starting": [], "core": [6697, 3047, 6676], "optional": [3036, 6673, 3031], "order": [6697, 3047, 6676, 3036, 6673, 3031]},
...
def build_champion_entry(champ_id: str, lane: str, ctx: dict | None,
    ...
    entry = {
        ...
        "build": {
            ...
            "order": resolve_items(know["build"]["order"], item_names) if know["build"].get("order") else None,
        },
        "combo": know["combo"],
    ...
    if know.get("rune_pages"):
        entry["rune_pages"] = [
            {
                "label": page["label"],
                "primary": resolve_runes(page["primary"], rune_map),
                "secondary": resolve_runes(page["secondary"], rune_map),
                "note": page.get("note", ""),
            }
            for page in know["rune_pages"]
        ]
    if lane == "JG":
